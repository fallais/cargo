...
func (s *SerialOBD) GetTotalKilometers(ctx context.Context) (int, error) {
	return pid(ctx, s, obd.PIDTotalKilometers.String(), ParseDistance)
}
...
func (s *SerialOBD) GetDTCs(ctx context.Context) ([]dtc.DTC, error) {
	elm, err := s.adapter()
	if err != nil {
		return nil, err
	}
	...
	for _, m := range dtcModes {
		...
		if err != nil {
			if errors.Is(err, ErrNoData) {
				// The module is there and has nothing to report for
				// this mode, which is the healthy case.
				answered = true
				continue
			}
			continue
